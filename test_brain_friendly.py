...
def test_increment_and_loop():
    assert eval_program('+>[]', 0, [0,0]) == [1,0]
    assert eval_program('[]>+', 0, [0,0]) == [0,1]
...
def test_nested_loops():
    ...
    # "Move value" program. Taken from Esolang Wiki
    # Moves the current cell two cells right.
    assert eval_program('>>[-]<<[->>+<<]', 0, [1,0,0]) == [0,0,1]
